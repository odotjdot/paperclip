...
// agy uses ~/.gemini/skills/ — same config directory as Gemini CLI
function resolveAgySkillsHome(config: Record<string, unknown>) {
  const env =
...
async function buildAgySkillSnapshot(config: Record<string, unknown>): Promise<AdapterSkillSnapshot> {
  ...
  return buildPersistentSkillSnapshot({
    ...
    missingDetail: "Configured but not currently linked into the agy skills home.",
